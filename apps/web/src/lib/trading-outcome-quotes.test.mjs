...
test("updates trading outcome prices from live last-trade and order-book quotes", () => {
  const outcomes = [
    {
      name: "Delhi Capitals",
      tokenId: "del-token",
  ...
  assert.equal(quoted[0].price, 0.45);
  assert.equal(quoted[0].probability, 45);
  assert.equal(quoted[1].price, 0.57);
  assert.equal(quoted[1].probability, 57);
});
