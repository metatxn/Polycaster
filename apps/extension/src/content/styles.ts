...
function injectInlineStyles(): void {
  try {
    ...
    style.textContent = `
      ...
      .knoww-card-footer svg,
      .knoww-outcome-btn svg,
      ...
      /* Main container - always fixed position on right side.
         z-index is pushed near INT32_MAX because some publishers (ESPN, for
         example) ship sticky/relative chrome at z-index ≥ 1000000, which
         would otherwise bury the panel. */
